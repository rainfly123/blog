...
static int get_server (char *ip, int *port, char *address)
{
    char *ntp = "getuploadserver.com";
    ...
    body_len = sprintf(body, "{'getserver':'client'}");
...
#define METHOD "POST /%s?md5=%s HTTP/1.0\r\n"
...
#define BOUNDARY_E "\r\n--------xiecc--\r\n"
static int http_post_jpg(char *file, char *ip, int port, char *address)
{
    ...
    len = sprintf(data, METHOD, address, md5);
...
int port;
char address[48];
get_server (ip, &port, address);
printf("%s %s %d\n", ip, address, port);
http_post_jpg("1407171636_0001.jpg" , ip, port, address);
}
*/
